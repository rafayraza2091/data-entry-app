...
export default function TaskEntryPage() {
  ...
  useEffect(() => {
    async function fetchData() {
      try {
        const [meRes, usersRes, subjRes, booksRes, chapRes, topRes] = await Promise.all([
          fetch('/api/auth/me'),
        ...
        if (usersRes.ok) {
          ...
          if (currentUser) {
            ...
            setReporter(ownerName); // Default reporter to the Owner
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Since we don't have ownerName saved in state, we can leave it blank on reset or if they change task,
      // but to be safe let's just leave it as whatever it currently is or blank.
      // Wait, let's just not reset the reporter after form submission so it stays as the owner!
  ...
  const availableTopics = topicsList.filter(t => t.subject === subject && t.book === book && t.chapterTitle === chapter || t.chapterName === chapter); // Chapter title field might vary
  
  // Extract unique topic names and exercises
  ...
  const handleTopicChange = (newTopic: string) => {
    setTopic(newTopic);
    // Find matching exercise for this topic
  ...
  const handleExerciseChange = (newExercise: string) => {
    ...
    // Find matching topic for this exercise
    ...
    <div className="glass-panel animate-slide-up" style={{ maxWidth: '800px', margin: '2rem auto', padding: '2rem' }}>
      ...
      <form onSubmit={handleSubmit}>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem', marginBottom: '1.5rem' }}>
          ...
          {/* Class Field (Always shown, Disabled) */}
          <div className="form-group">
          ...
          {/* Exercise dropdown is only shown if there are exercises available for the chosen book/chapter */}
